...
@contextmanager
def get_db_connection(max_attempts=5, timeout=60.0):
    conn = None
    attempt = 0
    backoff_time = 1.0
...
def migrate_database():
    with get_db_connection() as conn:
        ...
        # Create player_team_scores table if it doesn't exist
        ...
        # Check if player_team_changes has score_at_change column
        ...
        # Create player_team_changes if it doesn't exist (with needed columns)
        ...
        if not cursor.fetchone():
            ...
            # Check if is_redeem column exists
            cursor.execute("PRAGMA table_info(player_team_changes)")
            columns = cursor.fetchall()
            column_names = [col[1] for col in columns]
        ...
        # Update any null end times for inactive sessions
